...
- The direct connection to the database is handled by Open DataBase Connecstions (ODBC), for which R provides two major packages `odbc` and `RODBC`,
...
-{Benchmark Preconditions} I am running the benchmark on an (LINK!!!) RStudio instance with the size t2.micro (4GB of RAM), and a SQL Server database on a different instance (1GB RAM).
